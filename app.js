...
function encontrarChavePorId(id) {
  for (const chave of Object.keys(compatibilidadePorDroga)) {
...
function buscarCompatibilidade(idA, idB) {
  ...
  if (chaveA) {
    const info = compatibilidadePorDroga[chaveA];
  ...
  if (chaveB) {
    const info = compatibilidadePorDroga[chaveB];
...
function validarBase() {
  ...
  for (const chave of Object.keys(compatibilidadePorDroga)) {
    ...
    const info = compatibilidadePorDroga[chave];
  ...
  for (const med of medicamentos) {
    const temBloco = Object.keys(compatibilidadePorDroga).some(
    ...
    function popularSelects() {
      ...
      selects.forEach(select => {
        medicamentos.forEach(med => {
          ...
          opt.value = med.id;  // usamos o id interno do medicamento
          opt.textContent = med.nome; // nome de exibição do medicamento
...
for (let i = 0; i < unicos.length; i++) {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      popularSelects();
      validarBase(); // usar apenas durante testes//
